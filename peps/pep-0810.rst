...
``__import__``. ``__lazy_import__`` registers the module in an internal
tracking structure (accessible as a read-only snapshot via
:func:`!sys.get_lazy_modules`), and returns a
...
Lazy module objects do not appear in ``sys.modules``; they are tracked
internally and visible via :func:`!sys.get_lazy_modules`. Under normal
operation lazy objects should only end up stored in global variables, and the
common ways to access those variables (regular variable access, module
...
When the module is reified, it's removed from the internal tracking structure
(and therefore no longer visible in :func:`!sys.get_lazy_modules`), even if
...
* ``sys.get_lazy_modules()`` - Returns an immutable snapshot of pending lazy
  imports as a ``frozendict[str, frozenset[str]]`` mapping module names to the
  sets of their lazily imported submodule names. Primarily for diagnostics and
  introspection.
...
:func:`!sys.get_lazy_modules` and import state, so lazy imports in one
subinterpreter do not affect others.
